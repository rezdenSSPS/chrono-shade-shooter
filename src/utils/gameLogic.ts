...
export const checkPlayerEnemyCollisions = (gameData: GameData, setGameState: React.Dispatch<React.SetStateAction<GameUIState>>, channel?: RealtimeChannel) => {
            ...
            if (distance < player.size + enemy.size) {
                ...
                if (channel) {
                     channel.send({
                        type: 'broadcast',
                        event: 'player-hit',
                        payload: { playerId: player.id, newHealth: player.health, by: 'enemy' },
                    });
                }
            }
        }
...
export const checkPlayerBulletCollisions = (gameData: GameData, setGameState: React.Dispatch<React.SetStateAction<GameUIState>>, channel?: RealtimeChannel) => {
    const allPlayers = [gameData.player, ...gameData.otherPlayers];
    for (let i = gameData.bullets.length - 1; i >= 0; i--) {
        const bullet = gameData.bullets[i];
        if (!bullet) continue;
            ...
            if (distance < bullet.size + player.size) {
                ...
                if (channel) {
                     channel.send({
                        type: 'broadcast',
                        event: 'player-hit',
                        payload: { playerId: player.id, newHealth: player.health, by: bullet.playerId },
                    });
                }
